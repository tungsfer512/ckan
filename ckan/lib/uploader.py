...
class Upload(object):
    def __init__(self, object_type, old_filename=None):
        ...
        self.storage_path = os.path.join(
            path, "storage", "uploads", object_type
        )
    ...
    def verify_type(self):
        ...
        types = aslist(
            config.get(
                "ckan.upload.{}.types".format(self.object_type), ["image"]
            )
        )
...
class ResourceUpload(object):
    def __init__(self, resource):
        ...
        if bool(upload_field_storage) and isinstance(
            upload_field_storage, ALLOWED_UPLOAD_TYPES
        ):
            ...
            if not self.mimetype and config_mimetype_guess == "file_contents":
                try:
                    self.mimetype = magic.from_buffer(
                        self.upload_file.read(), mime=True
                    )
                    self.upload_file.seek(0, os.SEEK_SET)
                except IOError as e:
    ...
    def upload(self, id, max_size=10):
        ...
        if self.filename:
            ...
            minio_filepath = filepaths[-3] + filepaths[-2] + filepaths[-1]
            if bucket:
                client.fput_object(bucket_name, minio_filepath, filepath)
            else:
                raise logic.NotFound("Bucket does not exist")
            return
